                ...
                <div key={i} className="space-y-1">
                  <div className="relative min-h-[28px]">
                    {ln.chords.map((c) => (
                      <div
                        key={c.id}
                        className={`absolute -translate-y-1/2 top-1/2 text-xs px-2 py-0.5 rounded border cursor-grab active:cursor-grabbing whitespace-nowrap ${
                        ...
                        style={{
                          left: beatPx(c.startBeat - sec.startBeat, zoom),
                        }}
                  ...
                  <div className="relative min-h-[22px] text-[13px] tracking-wide font-medium text-slate-100">
                    {ln.lyrics.map((l) => (
                      <span
                        key={l.id}
                        className={`absolute -translate-y-1/2 top-1/2 px-1 rounded cursor-pointer ${
                          selection.kind === "lyric" && selection.id === l.id
                            ? "bg-slate-700/40"
                            : ""
                        }`}
                        style={{ left: beatPx(l.beat - sec.startBeat, zoom) }}
