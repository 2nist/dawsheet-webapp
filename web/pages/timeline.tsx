  ...
  return (
    <main className="p-6 min-h-screen bg-[#efe3cc]">
      ...
      <div className="border border-black/15 rounded-[6px] bg-white shadow-[0_1px_0_rgba(0,0,0,.25)] overflow-x-auto">
        <div className="relative" style={{ width: totalBeats * zoom, minHeight: 400 }}>
          ...
          <div className="absolute top-0 left-0 right-0 h-8 border-b border-black/10">
            {Array.from({ length: Math.ceil(totalBeats / 4) }).map((_, i) => (
              <div
                key={i}
                className="absolute top-0 border-l border-black/20 h-full flex items-center pl-1"
                style={{ left: i * 4 * zoom }}
              >
          ...
          <div className="absolute top-8 left-0 right-0 h-12 border-b border-black/10">
            {sections.map((section) => (
              <div
                key={section.id}
                className="absolute top-1 h-10 rounded border border-black/20 flex items-center justify-center"
                style={{
                  left: section.startBeat * zoom,
                  width: section.lengthBeats * zoom,
          ...
          <div className="absolute top-20 left-0 right-0 h-12 border-b border-black/10">
            {chords.map((chord, index) => (
              <div
                key={index}
                className="absolute top-1 h-10 bg-blue-100 border border-blue-300 rounded flex items-center justify-center"
                style={{
                  left: chord.startBeat * zoom,
                  width: Math.max(zoom * 2, 60),
                }}
              >
          ...
          <div className="absolute top-32 left-0 right-0 h-16">
            {lyrics.map((lyric, index) => (
              <div
                key={index}
                className="absolute top-1 bg-green-100 border border-green-300 rounded px-2 py-1"
                style={{
                  left: (lyric.beat || 0) * zoom,
                }}
              >
