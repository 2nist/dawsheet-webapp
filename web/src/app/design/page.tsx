...
const RADIUS_TOKENS: TokenName[] = [
  "--radius-sm",
  "--radius",
  "--radius-lg",
];
const SPACING_TOKENS: TokenName[] = [
  "--spacing-xs",
  "--spacing-sm",
  "--spacing-md",
  "--spacing-lg",
];
const FONT_TOKENS: TokenName[] = ["--font-sans", "--font-mono"];
const SHADOW_TOKENS: TokenName[] = [
  "--shadow-sm",
  "--shadow",
  "--shadow-lg",
];
...
export default function DesignPage() {
  ...
  function resolveVarToHex(varName: TokenName): string {
    ...
    const computed = getComputedStyle(el).color; // rgb(a)
    document.body.removeChild(el);
    return cssColorToHex(computed) || cssColorToHex(tokens[varName]) || "#000000";
  }
  ...
  return (
        ...
        <header className="flex items-end justify-between">
          <div>
            ...
            <p className="text-sm opacity-80">
              ...
              <code className="ml-1">src/styles/tokens.css</code> and are mapped
              via Tailwind.
            </p>
          </div>
        ...
        <section className="space-y-md">
          ...
          <div className="rounded-lg border border-border p-md">
            <div className="flex items-center gap-md mb-sm">
              <div>
                <label className="block text-sm opacity-80 mb-xs">BG token</label>
                <select
              ...
              <div>
                <label className="block text-sm opacity-80 mb-xs">FG token</label>
                <select
                  className="rounded border border-border bg-bg p-sm"
                  value={fgChoice}
              ...
              <div className="ml-auto text-sm opacity-80">
                ...
                {typeof contrast === "number" && (
                  <span className="ml-sm">
                    {contrast >= 7 ? "(AAA)" : contrast >= 4.5 ? "(AA)" : "(Fail)"}
                  </span>
                )}
              </div>
            </div>
              ...
              <div key={t} className="rounded-lg border border-border p-md">
                ...
                <div className="flex items-center gap-sm">
                  <input
                    ...
                    onChange={(e) => updateToken(t, toPx(Number(e.target.value)))}
                  />
            ...
            {SPACING_TOKENS.map((t) => (
              <div key={t} className="rounded-lg border border-border p-md">
                ...
                <input
                  ...
                  onChange={(e) => updateToken(t, toPx(Number(e.target.value)))}
                />
                <div className="mt-sm text-center bg-secondary/30" style={{ padding: `var(${t})` }}>
                  sample
                </div>
              </div>
            ))}
          ...
          <div className="grid sm:grid-cols-3 gap-md">
            {SHADOW_TOKENS.map((t) => (
              <div key={t} className="rounded-lg border border-border p-md">
                ...
                />
                <div className="h-16 bg-bg rounded" style={{ boxShadow: `var(${t})` }} />
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="space-y-sm">
          ...
          <div className="grid md:grid-cols-2 gap-md">
            ...
            <div className="space-x-sm">
              ...
              <button className="btn" style={{ background: "var(--secondary)" }}>
                Secondary
              </button>
            </div>
          </div>
    ...
    {modalOpen && (
      ...
      >
        <div className="rounded-lg border border-border p-md" style={{ background: "var(--bg)" }}>
          <div className="text-lg font-semibold mb-sm">Adjust HSL {modalToken}</div>
          <div className="grid gap-sm" style={{ minWidth: 280 }}>
            <label className="grid gap-xs">
              <span className="text-sm opacity-80">Hue: {hsl.h}</span>
              <input type="range" min={0} max={360} value={hsl.h} onChange={(e) => setHsl({ ...hsl, h: Number(e.target.value) })} />
            </label>
            <label className="grid gap-xs">
              <span className="text-sm opacity-80">Saturation: {hsl.s}%</span>
              <input type="range" min={0} max={100} value={hsl.s} onChange={(e) => setHsl({ ...hsl, s: Number(e.target.value) })} />
            </label>
            <label className="grid gap-xs">
              <span className="text-sm opacity-80">Lightness: {hsl.l}%</span>
              <input type="range" min={0} max={100} value={hsl.l} onChange={(e) => setHsl({ ...hsl, l: Number(e.target.value) })} />
            </label>
            <div className="h-12 rounded" style={{ background: hslToCss(hsl), border: "1px solid var(--border)" }} />
            <div className="flex justify-end gap-sm mt-sm">
              <button className="btn" type="button" onClick={() => setModalOpen(false)}>Cancel</button>
              <button className="btn" type="button" onClick={applyHsl}>Apply</button>
            </div>
          </div>
        </div>
      </div>
